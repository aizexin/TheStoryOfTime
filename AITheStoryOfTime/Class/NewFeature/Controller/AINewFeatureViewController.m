...
- (void)setupScrollView {
  UIScrollView *scrollV = [[UIScrollView alloc]
      initWithFrame:CGRectMake(0, 0, Mainsize.width, Mainsize.height)];
  ...
  scrollV.contentSize =
      CGSizeMake(ImageCount * Mainsize.width, Mainsize.height);
  scrollV.bounces = NO;
...
- (void)setupStartButton:(UIImageView *)imageView {
  ...
  //设置背景图片
  [starBtn setBackgroundImage:[UIImage imageNamed:@"new_feature_finish_button"]
                     forState:(UIControlStateNormal)];
  [starBtn setBackgroundImage:
  ...
  CGPoint center = CGPointMake(self.view.frame.size.width * 0.5,
  ...
  //添加点击事件
  [starBtn addTarget:self
                action:@selector(onClickStartBtn)
...
- (void)setupShareButton:(UIImageView *)imageView {
  ...
  CGPoint center = CGPointMake(self.view.frame.size.width * 0.5,
                               self.view.frame.size.height * 0.7);
  ...
  [shareBtn setTitleColor:[UIColor blackColor]
                 forState:(UIControlStateSelected)];
  //设置间距
...
- (void)onClickStartBtn {
  ...
  //    app.ddVC = [[DDMenuController
  //    alloc]initWithRootViewController:tabBarVC];
  //    //添加左边birth页面
...
- (void)setupPageControl {
  ...
  pageControl.currentPageIndicatorTintColor =
      AIColor(253, 98, 42);  // 当前页的小圆点颜色
  CGPoint centerPoint = CGPointMake(self.view.frame.size.width * 0.5,
